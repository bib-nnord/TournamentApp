...
function hasCapacity(
  tournament: { max_participants: number | null },
  participants: Array<{ registration_status: string }>,
): boolean {
  if (!tournament.max_participants) return true;
  const activeCount = participants.filter((participant) => countsAgainstCapacity(participant.registration_status)).length;
...
export async function createScheduled(req: Request, res: Response) {
  ...
  if (isTeamMode && inviteEntries.some((invite) => invite.type === 'team')) {
    return res.status(400).json({ error: 'Team mode tournaments only accept individual player invites, not existing team invites' });
  }
  ...
  try {
    ...
    for (const username of usernameInvites) {
      if (!userByUsername.has(username)) {
        return res.status(400).json({ error: `Invited account not found: ${username}` });
      }
    }
    for (const userId of userIdInvites) {
      if (!userById.has(userId)) {
        return res.status(400).json({ error: `Invited account not found: ${userId}` });
      }
    }
    for (const teamId of teamIdInvites) {
      if (!teamById.has(teamId)) {
        return res.status(400).json({ error: `Invited team not found: ${teamId}` });
      }
    }
    ...
    for (const invite of inviteEntries) {
      if (invite.type === 'team') {
        if (!Number.isInteger(invite.teamId)) {
          return res.status(400).json({ error: 'Team invite requires teamId' });
        }

        const team = teamById.get(Number(invite.teamId));
        if (!team) {
          return res.status(400).json({ error: `Invited team not found: ${invite.teamId}` });
        }
        ...
        if (seenDisplayNames.has(normalizedDisplay)) {
          return res.status(400).json({ error: `Duplicate participant display name: "${displayName}"` });
        }

        seenTeams.add(team.team_id);
        seenDisplayNames.add(normalizedDisplay);
        participantRecords.push({
          seed: participantRecords.length + 1,
      ...
      if (invite.type === 'account') {
        ...
        if (!resolvedUser) {
          return res.status(400).json({ error: 'Account invite requires a valid userId or username' });
        }
    ...
    if (maxParticipants !== undefined && participantRecords.length > maxParticipants) {
      return res.status(400).json({ error: 'Number of invites cannot exceed maxParticipants' });
    }
...
export async function registerScheduled(req: Request, res: Response) {
  ...
  try {
    ...
    if (tournament.team_mode && Number.isInteger(teamId)) {
      return res.status(400).json({ error: 'Team mode tournaments do not accept team registrations. Register as an individual player.' });
    }

    const mode = tournament.registration_mode;
    const registrationStatus = mode === 'open' ? 'approved' : 'pending';
    const confirmed = mode === 'open';
    ...
    if (Number.isInteger(teamId)) {
      ...
      if (!existingParticipant && !hasCapacity(tournament, tournament.participants)) {
...
async function updateParticipantDecision(
  ...
  try {
    ...
    if (decision === 'approved' && !countsAgainstCapacity(participant.registration_status) && !hasCapacity(tournament, tournament.participants)) {
      return res.status(400).json({ error: 'Tournament is full' });
    }
